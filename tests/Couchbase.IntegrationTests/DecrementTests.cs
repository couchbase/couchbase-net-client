    ...
    public class DecrementTests : IClassFixture<ClusterFixture>
    {
        ...
        public async Task Can_decrement_value()
        {
            ...
            try
            {
                ...
                var result = await collection.Binary.DecrementAsync(key, options => options.Initial(1)).ConfigureAwait(true);
        ...
        public async Task Can_specify_initial_and_delta_values()
        {
            ...
            {
                // doc doesn't exist, create it and use initial value (1)
                var result = await collection.Binary.DecrementAsync(key, options => options.Initial(5)).ConfigureAwait(true);
                Assert.Equal((ulong) 5, result.Content);

                // increment again, doc exists, increments to 6
                result = await collection.Binary.DecrementAsync(key, options => options.Delta(5)).ConfigureAwait(true);
                Assert.Equal((ulong) 0, result.Content);
            }
            finally
            {
        ...
        public async Task Test_Expiry()
        {
            ...
            {
                // doc doesn't exist, create it and use initial value (1)
                var result = await collection.Binary.DecrementAsync(key, options => options.Expiry(TimeSpan.FromMilliseconds(500)).Initial(1)).ConfigureAwait(true);

    ...
    ITestOutputHelper outputHelper)
{
    ...
    {
        var col = await fixture.GetDefaultCollection();
    ...
    public async Task Test_GetAsync()
    {
        ...
        Assert.Equal(doc1, (content?.SelectToken("name") ?? throw new InvalidOperationException()).Value<string>());
    }
    ...
    public async Task Test_ReplaceAsync()
    {
        ...
        Assert.Equal("changed", (content1?.SelectToken("name") ?? throw new InvalidOperationException()).Value<string>());
    }
    ...
    public async Task Test_GetAndLockAsync_Unlocked()
    {
        ...
        var result1 = await col.GetAsync(doc1);
        var content1 = result1.ContentAs<JObject>();
        Assert.Equal("Test_GetAndLockAsync", (content1?.SelectToken("name") ?? throw new InvalidOperationException()).Value<string>());
    }
    ...
    public async Task Test_MutateInAsync_Chained()
    {
        ...
        Assert.Equal("null", (content?.SelectToken("name") ?? throw new InvalidOperationException()).Value<string>());
        Assert.Equal(2, (content.SelectToken("insert") ?? throw new InvalidOperationException()).Value<int>());
    ...
    public async Task Test_MutateInAsync_ArrayAddUnique()
    {
        ...
        var result1 = await col.GetAsync(doc1);
        var content = result1.ContentAs<JObject>();
        Assert.Contains(9, content?.SelectToken("items").ToObject<List<int>>() ?? throw new InvalidOperationException());
    }
    ...
    public async Task Test_MutateInAsync_ArrayInsert()
    {
        ...
        var result1 = await col.GetAsync(doc1);
        var content = result1.ContentAs<JObject>();
        Assert.Contains(8, content?.SelectToken("items")?.ToObject<List<int>>() ?? throw new InvalidOperationException());
        Assert.Contains(9, content?.SelectToken("items")?.ToObject<List<int>>() ?? throw new InvalidOperationException());
    }
    ...
    public async Task Test_MutateInAsync_SetDoc()
    {
        ...
        var result1 = await col.GetAsync(doc1);
        var content = result1.ContentAs<JObject>();
        Assert.Equal("Days", (content.SelectToken("strange") ?? throw new InvalidOperationException()).Value<string>());
    }
    ...
    public async Task Test_MutateInAsync_Increment()
    {
        ...
        var result1 = await col.GetAsync(doc1);
        var content = result1.ContentAs<JObject>();
        Assert.Equal(3, (content?.SelectToken("counter") ?? throw new InvalidOperationException()).Value<int>());
    }
    ...
    public async Task Test_MutateInAsync_Decrement()
    {
        ...
        var result1 = await col.GetAsync(doc1);
        var content = result1.ContentAs<JObject>();
        Assert.Equal(1, (content?.SelectToken("counter") ?? throw new InvalidOperationException()).Value<int>());
    }
    ...
    public async Task Test_GetAnyReplica_Throws_DocumentUnretrievable()
    {
        ...
        var collection = await fixture.GetDefaultCollection();
    ...
    public async Task Test_KV_Operation_Properly_Records_Its_Latency()
    {
        ...
        var scope = await fixture.GetDefaultScope();

        // Create and get collection on default bucket/scope
        await bucket.Collections.CreateCollectionAsync("_default", collectionName, new CreateCollectionSettings());
        await _testHelper.WaitUntilCollectionIsPresent(collectionName);
        var collection = await scope.CollectionAsync(collectionName);

        //Upsert a doc so that the client caches the Cid
        await collection.UpsertAsync(id, new { Content = "hello" });
        await _testHelper.WaitUntilDocumentIsPresent(id, collectionName);

        // Drop collection
        await bucket.Collections.DropCollectionAsync("_default", collectionName);
        await _testHelper.WaitUntilCollectionIsDropped(collectionName);

        var stopwatch = Stopwatch.StartNew();
        var exception = await Record.ExceptionAsync(async () => await collection.GetAsync(id));
        stopwatch.Stop();

        outputHelper.WriteLine("Exception: " + exception!.Message);
        outputHelper.WriteLine($"Test measured Time for operation = {stopwatch.Elapsed.TotalSeconds}");

        Assert.Equal(typeof(UnambiguousTimeoutException), exception!.GetType());
        Assert.True(exception.Message.Contains("The Get operation") || exception.Message.Contains("The GetCidByName operation"));
        Assert.Contains("00:00:02.5", exception.Message);
    }

...
namespace Couchbase.Tests.IO.Operations
{
    ...
    public class ReplaceTests : OperationTestBase
    {
        ...
        public void When_Document_Exists_Replace_Succeeds()
        {
            ...
            //add the new doc
            var add = new Add<dynamic>(key, new { foo = "foo" }, GetVBucket(), Converter, Transcoder, OperationLifespanTimeout);
            var result1 = IOStrategy.Execute(add);
            Assert.IsTrue(result1.Success);

            //replace it the old doc with a new one
            var replace = new Replace<dynamic>(key, new { bar = "bar" }, GetVBucket(), Converter, Transcoder, OperationLifespanTimeout);

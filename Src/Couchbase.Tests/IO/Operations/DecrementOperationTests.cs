...
namespace Couchbase.Tests.IO.Operations
{
    ...
    public class DecrementOperationTests : OperationTestBase
    {
        ...
        public void Test_DecrementOperation()
        {
            ...
            var delete = new DeleteOperation(key, GetVBucket(), new ManualByteConverter(), new TypeSerializer2(new ManualByteConverter()));
            var result = IOStrategy.Execute(delete);
            Console.WriteLine("Deleting key {0}: {1}", key, result.Success);

            //increment the key
            var operation = new IncrementOperation(key, 1, 1, 0, GetVBucket(), new ManualByteConverter(), new TypeSerializer2(new ManualByteConverter()));
            var result1 = IOStrategy.Execute(operation);
            Assert.IsTrue(result1.Success);
            Assert.AreEqual(result1.Value, 1);

            //key should be 1
            var get = new GetOperation<string>(key, GetVBucket(), new ManualByteConverter(), new TypeSerializer2(new ManualByteConverter()));
            var result3 = IOStrategy.Execute(get);
            Assert.AreEqual(result1.Value.ToString(CultureInfo.InvariantCulture), result3.Value);

            //decrement the key
            var decrement = new DecrementOperation(key, 1, 1, 0, GetVBucket(), new ManualByteConverter(), new TypeSerializer2(new ManualByteConverter()));
            var result2 = IOStrategy.Execute(decrement);
            Assert.IsTrue(result2.Success);
            Assert.AreEqual(result2.Value, 0);

            //key should be 0
            get = new GetOperation<string>(key, GetVBucket(), new ManualByteConverter(), new TypeSerializer2(new ManualByteConverter()));
